...
class CiscoSSHClient:
    ...
    def _configure_ssh_client_with_legacy_algorithms(self):
        """Cấu hình SSH client với các thuật toán legacy cho thiết bị Cisco cũ."""
        # Thêm hỗ trợ cho các thuật toán host key cũ
        ...
        # Cấu hình thuật toán mã hóa và MAC cho thiết bị cũ
    ...
    def connect_with_legacy_support(self):
        """Thiết lập kết nối SSH với hỗ trợ các giao thức legacy cho thiết bị Cisco cũ."""
        try:
            self.client = paramiko.SSHClient()
            self.client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            
            # Cấu hình các thuật toán legacy
            self._configure_ssh_client_with_legacy_algorithms()

            # Thử kết nối để kiểm tra các phương thức xác thực có sẵn
            try:
                transport = paramiko.Transport((self.hostname, self.port))
                transport.connect()
                
                # Kiểm tra các phương thức xác thực có sẵn
                available_auths = transport.auth_none(self.username)
                print(f"Available authentication methods: {available_auths}", file=sys.stderr)
                
                # Thử password authentication nếu có sẵn
                if 'password' in str(available_auths).lower():
                    try:
                        transport.auth_password(self.username, self.password)
                        self.client._transport = transport
                        print(f"SSH connected using password authentication with legacy support to {self.hostname}.", file=sys.stderr)
                        return True
                    except paramiko.AuthenticationException:
                        print(f"Password authentication failed even though it's listed as available.", file=sys.stderr)
                
                # Thử keyboard-interactive nếu có sẵn
                if 'keyboard-interactive' in str(available_auths).lower():
                    try:
                        transport.auth_interactive(self.username, self._keyboard_interactive_handler)
                        self.client._transport = transport
                        print(f"SSH connected using keyboard-interactive authentication with legacy support to {self.hostname}.", file=sys.stderr)
                        return True
                    except paramiko.AuthenticationException as e:
                        print(f"Keyboard-interactive authentication failed: {e}", file=sys.stderr)
                
                # Thử none authentication (có thể switch cho phép kết nối mà không cần xác thực)
                try:
                    transport.auth_none(self.username)
                    if transport.is_authenticated():
                        self.client._transport = transport
                        print(f"SSH connected using none authentication with legacy support to {self.hostname}.", file=sys.stderr)
                        return True
                except paramiko.AuthenticationException:
                    pass
                
                print(f"All authentication methods failed. Available methods were: {available_auths}", file=sys.stderr)
                transport.close()
                return False
                
            except Exception as e:
                print(f"Error during transport connection: {e}", file=sys.stderr)
                return False
            
        except Exception as e:
            print(f"Error: SSH connection with legacy support failed to {self.hostname}: {e}", file=sys.stderr)
            self.client = None
            return False

    def connect(self):
        """Thiết lập kết nối SSH ban đầu, thử các phương thức xác thực."""
        # Thử kết nối với legacy support trước
        if self.connect_with_legacy_support():
            return True
            
        # Nếu không thành công, thử phương thức kết nối gốc với debug
        try:
            self.client = paramiko.SSHClient()
            self.client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

            # Thử với transport trực tiếp để có thêm thông tin debug
            try:
                transport = paramiko.Transport((self.hostname, self.port))
                transport.connect()
                ...
                # Kiểm tra các phương thức xác thực có sẵn
                available_auths = transport.auth_none(self.username)
                print(f"Fallback - Available authentication methods: {available_auths}", file=sys.stderr)
                
                # Thử các phương thức xác thực
                if 'password' in str(available_auths).lower():
                    try:
                        transport.auth_password(self.username, self.password)
                        self.client._transport = transport
                        ...
                        print(f"Fallback password authentication failed.", file=sys.stderr)
                ...
                if 'keyboard-interactive' in str(available_auths).lower():
                    try:
                        ...
                        print(f"Fallback keyboard-interactive authentication failed: {e}", file=sys.stderr)
                
                transport.close()
                return False
                
            except Exception as e:
                print(f"Error during fallback transport connection: {e}", file=sys.stderr)
                return False
